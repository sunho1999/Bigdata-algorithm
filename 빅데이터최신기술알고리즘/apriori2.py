...
def association_rule(freq_itemsets_all,cnt):
    while(True):
        for i in freq_itemsets_all:
            for k in itertools.combinations(i,cnt): # 부분집합
                k = frozenset(k)
                support_i = 0
                support_j = 0
                for j in freq_itemsets_all:
                    if k == j.intersection(k):
                        support_i +=1
                    if j == i:
                        continue
                    elif j == j.intersection(i):
                        support_j +=1
...
association_rule(list(freq_itemsets_all),1)
